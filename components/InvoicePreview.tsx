...
import { Reservation, Room, ReservationStatus, Client, BoardType, BoardConfiguration, Tax, HotelSettings } from '../types';
...
const InvoicePreview: React.FC<InvoicePreviewProps> = ({
    ...
    const [hotelInfo, setHotelInfo] = useState<HotelSettings | null>(null);
    ...
    const getTvaRateFor = (category: 'accommodation' | 'services'): number => {
        ...
        return 0; // Plus de fallback par défaut, on respecte la configuration
    };
    ...
    const invoiceLines = useMemo<InvoiceLine[]>(() => {
        return reservationsToBill.flatMap(res => {
            ...
            // Ajout des taxes fixes (ex: Taxe de séjour)
            const activeFixedTaxes = availableTaxes.filter(t => t.isActive && t.isFixed && (t.applyTo === 'accommodation' || t.applyTo === 'all'));
            activeFixedTaxes.forEach(tax => {
                lines.push({
                    id: `${res.id}-tax-${tax.id}`,
                    date: res.checkIn,
                    description: `${tax.name} (${res.adults} PAX)`,
                    quantity: nights * res.adults,
                    unitPrice: tax.rate,
                    total: nights * res.adults * tax.rate,
                    tvaRate: 0 // Pas de TVA sur une taxe fixe
                });
            });
    ...
    return (
        <div className="fixed inset-0 z-[1000] bg-[#0f172a] flex flex-col no-print-background print:static print:bg-white overflow-hidden">
            ...
            <div ref={containerRef} className="flex-1 overflow-auto p-12 flex justify-center items-start bg-[#0f172a] no-scrollbar scroll-smooth">
                ...
                >
                    <div className="flex justify-between items-start mb-6 border-b border-indigo-100 pb-6">
                        <div className="space-y-4">
                            <div className="flex items-center gap-4">
                                {hotelInfo?.logo ? (
                                    <img src={hotelInfo.logo} alt="Logo" className="h-12 object-contain" />
                                ) : (
                                    <div className="text-indigo-600"><Hotel size={32} strokeWidth={2.5} /></div>
                                )}
                                <div>
                                    <h1 className="text-xl font-black text-slate-900 tracking-tight uppercase leading-none">{hotelInfo?.name || 'Hotel Manager'}</h1>
                                    <p className="text-[9px] font-bold text-slate-400 uppercase tracking-[0.3em] mt-2">Établissement Hôtelier</p>
                                </div>
                            </div>
                            <div className="text-[10px] leading-relaxed text-slate-500 font-medium pt-2">
                                <p className="font-bold text-slate-800 uppercase tracking-tight mb-1">{hotelInfo?.name}</p>
                                <p>{hotelInfo?.address || 'Adresse de l\'établissement'}</p>
                                <p>Tél : {hotelInfo?.phone || '+XX XXX XXX XXX'} | {hotelInfo?.email || 'contact@hotel.com'}</p>
                                {hotelInfo?.siret && <p className="uppercase tracking-widest text-[9px] mt-1 opacity-70">I.F. / SIRET : {hotelInfo.siret}</p>}
                            </div>
                        </div>

                        <div className="text-right">
                            <h2 className="text-3xl font-black text-slate-900 uppercase tracking-tighter">{isProforma ? 'Facture Proforma' : 'Facture'}</h2>
                            ...
                            <div className="mt-2 pt-2 border-t border-slate-50 text-[9px] font-bold uppercase text-slate-500 tracking-widest">
                    ...
                    <div className="mb-6 flex justify-end">
                        <div className="w-1/2 p-4 rounded-2xl border border-slate-100 bg-slate-50/50">
                            ...
                            <div className="mt-3 pt-3 border-t border-slate-200/50 flex items-center gap-3">
                    ...
                    <div className="flex-1">
                        <table className="w-full text-left mb-8">
                            <thead className="border-b-2 border-slate-900">
                                <tr>
                                    <th className="py-2 font-black text-[9px] uppercase tracking-widest text-slate-400">Libellé</th>
                                    <th className="py-2 text-center w-20 font-black text-[9px] uppercase tracking-widest text-slate-400">Qté</th>
                                    <th className="py-2 text-right w-24 font-black text-[9px] uppercase tracking-widest text-slate-400">P.U. HT</th>
                                    <th className="py-2 text-right w-32 font-black text-[9px] uppercase tracking-widest text-slate-400">Total TTC</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-100">
                                {invoiceLines.map((line, idx) => (
                                    <tr key={idx}>
                                        ...
                                        <td className="py-1.5 text-center text-[11px] font-black text-slate-500 tabular-nums">{line.quantity}</td>
                                        <td className="py-1.5 text-right text-[11px] font-bold text-slate-500 tabular-nums">{formatPrice(line.unitPrice / (1 + line.tvaRate))}</td>
                                        <td className="py-1.5 text-right text-[11px] font-black text-slate-900 tabular-nums">{formatPrice(line.total)}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
                            ...
                            <div className="mt-6 bg-slate-50/50 border border-slate-100 rounded-2xl p-4">
                                ...
                                <table className="w-full text-left">
                                    <thead className="border-b border-slate-200">
                                        <tr>
                                            <th className="py-1.5 text-[8px] font-black text-slate-400 uppercase tracking-widest">Date</th>
                                            <th className="py-1.5 text-[8px] font-black text-slate-400 uppercase tracking-widest">Désignation</th>
                                            <th className="py-1.5 text-[8px] font-black text-slate-400 uppercase tracking-widest">Mode</th>
                                            <th className="py-1.5 text-right text-[8px] font-black text-slate-400 uppercase tracking-widest">Montant</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-slate-100">
                                        {allPayments.map((p, idx) => (
                                            <tr key={idx}>
                                                <td className="py-1.5 text-[10px] text-slate-500 tabular-nums">{format(new Date(p.date), 'dd/MM/yyyy')}</td>
                                                <td className="py-1.5 text-[10px] font-bold text-slate-700">
                                                    {p.isDeposit ? `Acompte initial (Ch.${p.roomNumber})` : `Versement partiel (Ch.${p.roomNumber})`}
                                                </td>
                                                <td className="py-1.5"><span className="text-[9px] font-black text-slate-400 border border-slate-200 px-1.5 py-0.5 rounded uppercase">{p.method}</span></td>
                                                <td className="py-1.5 text-right text-[10px] font-black text-emerald-600 tabular-nums">-{formatPrice(p.amount)}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                        ...
                        <div className="w-1/3">
                            ...
                            <div className="space-y-2">
                                {Object.entries(vatBreakdown).map(([rate, data]) => {
                                    ...
                                    if (vatData.baseHT === 0 || ratePercent === 0) return null;
                                    return (
                                        <div key={rate} className="flex justify-between text-[10px] text-slate-500 font-bold">
                                            ...
                                            <span>HT: {formatPrice(vatData.baseHT)}</span>
                                        </div>
                                    );
                                })}
                            </div>
                            ...
                            <div className="bg-slate-900 text-white p-6 rounded-3xl shadow-2xl relative overflow-hidden">
                                ...
                                <div className="space-y-2 mb-6">
                                    <div className="flex justify-between text-[10px] font-black text-slate-400 uppercase tracking-widest">
                                        <span>Total HT</span>
                                        <span className="font-mono">{formatPrice(totalHT)}</span>
                                    </div>
                    ...
                    {hotelInfo?.invoiceFooter && (
                        <div className="mt-6 text-center border-t border-slate-100 pt-4">
                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-relaxed whitespace-pre-line px-12 italic">
                                {hotelInfo.invoiceFooter}
                            </p>
                        </div>
                    )}
