...
const NewReservationModal: React.FC<NewReservationModalProps> = ({ isOpen, onClose, onSuccess, rooms, existingReservations, initialData }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      api.fetchCurrencySettings().then(setCurrency);
                      ...
                      <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-10 gap-3">
                          ...
                          return (
                              <button key={room.id} onClick={() => setSelectedRoomIds(prev => isSelected ? prev.filter(i => i !== room.id) : [...prev, room.id])} className={`aspect-square rounded-2xl border-2 transition-all flex flex-col items-center justify-center relative ${isSelected ? 'bg-indigo-600 border-indigo-700 text-white shadow-xl scale-105 z-10' : 'bg-slate-50 border-slate-50 text-slate-700 hover:border-indigo-300 hover:bg-white'}`}>
                                  ...
                                  <div className={`mt-1 text-[8px] font-black uppercase px-1 rounded ${isSelected ? 'bg-white/20' : 'bg-white text-emerald-600 border border-emerald-100'}`}>{formatPrice(price, currency)}</div>
               ...
               <div className="lg:col-span-7 space-y-6">
                  ...
                  <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200 space-y-6">
                     ...
                     <div className="grid grid-cols-2 gap-4 border-t pt-6 border-slate-100">
                        <div className="col-span-1"><label className={labelClass}><Coins size={14}/> Acompte</label><input type="number" step="0.01" className={`${inputClass} !bg-white`} value={depositAmount || ''} onChange={e => setDepositAmount(parseFloat(e.target.value) || 0)} placeholder="0.00"/></div>
                  ...
                  <div className="bg-slate-900 rounded-[2.5rem] shadow-2xl flex flex-col h-full overflow-hidden text-white">
                      <div className="p-8 flex-1">
                          ...
                          <div className="space-y-4 mb-10">
                             <div className="flex justify-between items-end"><div><p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Prestations</p><p className="text-xs text-slate-300 font-medium">Hébergement + Repas ({nights} nuits)</p></div><span className="font-black text-lg">{formatPrice(totalPrice - availableTaxes.filter(t=>t.isActive && t.isFixed).reduce((s,t)=>s+(t.rate*guests*nights), 0), currency)}</span></div>
                             {availableTaxes.filter(t => t.isActive && t.isFixed).length > 0 && (
                                <div className="flex justify-between items-end border-t border-white/5 pt-4"><div><p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Taxes Locales</p><p className="text-xs text-slate-300 font-medium">Taxes de séjour / Fixes</p></div><span className="font-black text-lg text-indigo-400">{formatPrice(availableTaxes.filter(t=>t.isActive && t.isFixed).reduce((s,t)=>s+(t.rate*guests*nights), 0), currency)}</span></div>
                             )}
                          </div>
                          {/* Fixed: Changed totalStayPrice to totalPrice as totalStayPrice was not defined in this scope */}
                          <div className="mt-auto border-t border-white/10 pt-8"><div className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.3em] mb-2">Total TTC estimé</div><div className="text-5xl font-[1000] tracking-tighter leading-none flex items-baseline gap-2 mb-4">{formatPrice(totalPrice, currency)}</div></div>
                      </div>
