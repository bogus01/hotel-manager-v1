...
const QuickReservationModal: React.FC<QuickReservationModalProps> = ({ isOpen, onClose, onSuccess, rooms, existingReservations, initialData }) => {
  ...
  }, [selectedRoomIds, rooms, nights, boardType, boardPrices, totalPax]);
                          ...
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                {isConflicting ? (
                                    ...
                                    <div className="flex items-center gap-2 text-[8px] font-bold text-slate-400 uppercase tracking-widest"><Users size={10}/> {calc.paxAllocated} Pers. <span className="text-emerald-600 font-black">{calc.nightlyPrice}€</span></div>
                                )}
                            </div>
                          </div>
              ...
              <section className={panelClass}>
                ...
                <div className="p-6 space-y-6">
                  <div className="grid grid-cols-12 gap-4">
                    ...
                    <div className="col-span-4"><label className={labelClass}>Acompte (€)</label><input type="number" className={inputClass} value={depositAmount || ''} onChange={e => setDepositAmount(parseFloat(e.target.value) || 0)} placeholder="0.00" /></div>
                      ...
                      <div key={idx} className="space-y-1 border-b border-white last:border-0 pb-2">
                        <div className="flex justify-between items-center text-[10px] font-bold">
                            <div className="flex flex-col">
                                ...
                                <span className="text-[8px] text-slate-400 uppercase font-bold">{nights} nuits x {item?.baseNightlyPrice.toFixed(2)}€</span>
                            </div>
                            <span className="text-slate-700 font-black">{(item?.totalStay || 0).toFixed(2)} €</span>
                        </div>
                        {item?.boardComponent! > 0 && (
                            <div className="flex justify-between items-center text-[9px] italic text-slate-500 pl-4">
                                ...
                                <span>+{item?.boardComponent.toFixed(2)} €</span>
                            </div>
                        )}
                      </div>
                    ))}
                  ...
                  <div className={`rounded-xl p-6 flex items-center justify-between shadow-2xl border-b-4 transition-all ${hasCollision ? 'bg-red-50 border-red-500 text-red-900' : 'bg-slate-800 text-white border-indigo-500'}`}>
                    <div>
                        ...
                        <p className="text-3xl font-[1000] tracking-tighter tabular-nums">{totalStayPrice.toFixed(2)} €</p>
                    </div>
                        ...
                        {availableRooms.map(room => {
                            ...
                            return (
                                <button key={room.id} onClick={() => setSelectedRoomIds(prev => isSelected ? prev.filter(id => id !== room.id) : [...prev, room.id])} className={`p-3 rounded-xl border-2 transition-all text-center relative flex flex-col items-center justify-center min-h-[100px] ${isSelected ? 'border-indigo-600 bg-indigo-50 shadow-xl scale-[1.05] z-10' : 'border-white bg-white hover:border-indigo-200 shadow-sm'}`}>
                                    ...
                                    <div className="mt-auto pt-2 border-t border-slate-100 w-full flex items-center justify-between"><span className="text-[10px] font-black text-emerald-600">{room.baseRate}€</span><div className="flex items-center gap-1 text-slate-400"><User size={10}/><span className="text-[10px] font-black">{room.capacity}</span></div></div>
