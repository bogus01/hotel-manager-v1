...
const ReservationModal: React.FC<ReservationModalProps> = ({ reservation, allReservations, isOpen, onClose, onSave, rooms }) => {
    ...
    // Initialisation et synchronisation des données de la chambre active
    useEffect(() => {
        ...
        // IMPORTANT : On n'écrase formData que si on change de chambre (id différent)
        // ou si aucune donnée n'est encore chargée. Cela empêche les rafraîchissements 
        // globaux (allReservations) de Planning d'écraser nos modifs locales (services/paiements).
        if (res && (!formData || formData.id !== activeResId)) {
            setFormData({ ...res });
    ...
    const handleDeletePayment = async (resId: string, paymentId: string) => {
        setIsProcessing(true);
        try {
            await api.deletePayment(paymentId, resId);
            await onSave(null);
            setIsConfirmingDeletePayment(null);
        } catch (e) {
    ...
    const handleAddPayment = async () => {
        ...
        try {
            ...
            // Mettre à jour l'état local immédiatement pour éviter l'écrasement par stale data
            const updatedPayments = [...(formData.payments || []), pay];
    ...
    const handleAddService = async () => {
        ...
        try {
            ...
            const updatedServices = [...(formData.services || []), item];
            const updatedTotal = Number(formData.totalPrice) + (item.price * item.quantity);
            setFormData({ ...formData, services: updatedServices, totalPrice: updatedTotal });
    ...
    const handleDeleteService = async (serviceId: string) => {
        ...
        try {
            ...
            // Mettre à jour l'état local immédiatement
            const svc = formData.services.find(s => s.id === serviceId);
            if (svc) {
                const updatedServices = formData.services.filter(s => s.id !== serviceId);
                const updatedTotal = Math.max(0, Number(formData.totalPrice) - (svc.price * svc.quantity));
                setFormData({ ...formData, services: updatedServices, totalPrice: updatedTotal });
            }
        ...
        try {
            ...
            for (const res of sortedGroupRes) {
                ...
                if (resBalance > 0.01) {
                    ...
                    const pay: Payment = { id: crypto.randomUUID(), amount: payAmount, date: new Date(), method: newPayment.method };
            ...
            if (remainingPayment > 0.01 && groupRes.length > 0) {
                ...
                const pay: Payment = { id: crypto.randomUUID(), amount: remainingPayment, date: new Date(), method: newPayment.method };
    ...
    return (
        <>
                            ...
                            <div className="w-72 bg-slate-50 border-r border-slate-100 flex flex-col shrink-0 overflow-y-auto">
                                ...
                                <div className="p-4 mt-auto border-t border-slate-100 bg-slate-100/50">
                                    <div className="bg-slate-800 text-white p-4 rounded-xl shadow-lg border-b-2 border-indigo-50">
                                        ...
                                        <span className="text-lg font-[1000] tracking-tighter tabular-nums">{formatPrice(groupFinancials.totalTTC)}</span>
                                                            ...
                                                            {groupRes.map(res => {
                                                                ...
                                                                return (
                                                                    <tr key={res.id} className="hover:bg-slate-50 group cursor-pointer" onClick={() => setActiveResId(res.id)}>
                                                                        ...
                                                                        <td className="px-6 py-5 text-right font-bold text-slate-900">{formatPrice(res.totalPrice)}</td>
                                                                        <td className="px-6 py-5 text-right font-bold text-emerald-600">-{formatPrice(resPaid)}</td>
                                                                        <td className="px-6 py-5 text-right"><span className={`font-black px-2 py-1 rounded-lg ${resBalance > 0.05 ? 'text-red-500 bg-red-50' : 'text-emerald-500 bg-emerald-50'}`}>{formatPrice(resBalance)}</span></td>
                                                                    </tr>
                                                                );
                                                            })}
                                                        </tbody>
                                                        <tfoot className="bg-slate-900 text-white font-black">
                                                            <tr><td colSpan={2} className="px-6 py-5 uppercase text-[10px] tracking-[0.2em] text-indigo-400">Totaux Dossier</td><td className="px-6 py-5 text-right">{formatPrice(groupFinancials.totalTTC)}</td><td className="px-6 py-5 text-right text-emerald-400">-{formatPrice(groupFinancials.totalPaid)}</td><td className={`px-6 py-5 text-right ${groupFinancials.balance > 0.05 ? 'text-red-400' : 'text-emerald-400'}`}>{formatPrice(groupFinancials.balance)}</td></tr>
                                                        </tfoot>
                                                    </table>
                                                </div>
                                            </section>
                                                        ...
                                                        <div className="divide-y divide-slate-100">
                                                            {groupFinancials.paymentsHistory.map((p, idx) => (
                                                                <div key={idx} className="p-4 hover:bg-slate-50 flex items-center justify-between group/pay transition-all">
                                                                    ...
                                                                    <div className="flex items-center gap-3"><div className="text-sm font-black text-emerald-600">-{formatPrice(p.amount)}</div>{p.method !== 'Acompte' && (
                                ...
                                <div className="flex flex-col gap-4 animate-in fade-in slide-in-from-right-4 duration-300">
                                    ...
                                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                                        ...
                                        <div className="space-y-4">
                                                        ...
                                                        <table className="w-full text-left border-separate border-spacing-0">
                                                            <thead className="bg-slate-50 sticky top-0 z-10 shadow-sm">
                                                                <tr>
                                                                    ...
                                                                    <th className="px-2 py-2 text-right w-20 font-black text-[8px] uppercase text-slate-400 tracking-widest">P.U.</th>
                                                                    <th className="px-2 py-2 text-center w-10 font-black text-[8px] uppercase text-slate-400 tracking-widest">Qté</th>
                                                                    <th className="px-4 py-2 text-right w-28 font-black text-[8px] uppercase text-slate-400 tracking-widest">Total</th>
                                                            ...
                                                            <tbody className="divide-y divide-slate-100">
                                                                <tr className="bg-indigo-50/30">
                                                                    <td className="px-4 py-3">
                                                                        <div className="flex flex-col gap-0.5">
                                                                            ...
                                                                            <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{nights} nuits x {formatPrice(formData?.baseRate || 0)}</span>
                                                                        </div>
                                                                    </td>
                                                                    <td className="px-2 py-3 text-right text-[10px] font-black text-slate-500 whitespace-nowrap">{formatPrice(formData?.baseRate || 0)}</td>
                                                                    <td className="px-2 py-3 text-center font-black text-xs text-slate-600">{nights}</td>
                                                                    <td className="px-4 py-3 text-right font-black text-xs text-slate-900 whitespace-nowrap">{formatPrice(formData!.baseRate * nights)}</td>
                                                                    ...
                                                                    <tr className="bg-emerald-50/20">
                                                                        <td className="px-4 py-3">
                                                                            <div className="flex flex-col gap-0.5">
                                                                                ...
                                                                                <span className="text-[8px] font-bold text-slate-400 uppercase tracking-widest">{nights} nuits x {formData?.adults} PAX x {formatPrice(currentBoardDailyPrice)}</span>
                                                                            </div>
                                                                        </td>
                                                                        <td className="px-2 py-3 text-right text-[10px] font-black text-slate-500 whitespace-nowrap">{formatPrice(currentBoardDailyPrice)}</td>
                                                                        <td className="px-2 py-3 text-center font-black text-xs text-slate-600">{nights * (formData?.adults || 1)}</td>
                                                                        <td className="px-4 py-3 text-right font-black text-xs text-slate-900 whitespace-nowrap">{formatPrice(currentBoardDailyPrice * nights * (formData?.adults || 1))}</td>
                                                                        {!isReadOnly && <td className="px-2"></td>}
                                                                    </tr>
                                                                )}
                                                                {formData?.services.map(svc => (
                                                                    <tr key={svc.id} className="hover:bg-slate-50 group transition-colors">
                                                                        ...
                                                                        <td className="px-2 py-2.5 text-right text-[10px] font-bold text-slate-500 whitespace-nowrap">{formatPrice(svc.price)}</td>
                                                                        <td className="px-2 py-2.5 text-center text-xs font-black text-slate-600">{svc.quantity}</td>
                                                                        <td className="px-4 py-2.5 text-right font-black text-xs text-slate-700 whitespace-nowrap">{formatPrice(svc.price * svc.quantity)}</td>
                                                                        {!isReadOnly && (
                                                            ...
                                                            <div className="grid grid-cols-12 gap-3 items-end">
                                                                <div className="col-span-6">
                                                                    <select className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 text-[11px] font-bold text-slate-700 outline-none" value={selectedCatalogId} onChange={e => { setSelectedCatalogId(e.target.value); const item = catalog.find(c => c.id === e.target.value); if (item) setNewService({ name: item.name, price: item.defaultPrice, quantity: 1 }); }}>
                                                                        ...
                                                                        {catalog.map(c => <option key={c.id} value={c.id}>{c.name} ({formatPrice(c.defaultPrice)})</option>)}
                                                                    </select>
                                                                </div>
                                            ...
                                            <section className={panelClass}>
                                                ...
                                                <div className="p-5">
                                                    <div className="space-y-2 mb-4 max-h-32 overflow-y-auto">
                                                        {formData?.payments.map(p => (
                                                            <div key={p.id} className="flex justify-between items-center bg-emerald-50 border border-emerald-100 rounded-lg p-2 transition-all group/p">
                                                                ...
                                                                <div className="flex items-center gap-2">
                                                                    <span className="font-black text-emerald-600 text-xs">-{formatPrice(p.amount)}</span>
                                                        ...
                                                        {formData!.depositAmount > 0 && <div className="flex justify-between items-center bg-blue-50 border border-blue-100 rounded-lg p-2"><span className="text-[9px] font-black text-blue-700 uppercase italic">Acompte initial</span><span className="font-black text-blue-600 text-xs">-{formatPrice(formData!.depositAmount)}</span></div>}
                                                    </div>
                                                    {!isReadOnly && (
                                            ...
                                            <div className="bg-slate-900 text-white rounded-2xl p-6 shadow-2xl relative overflow-hidden"><div className="absolute top-0 right-0 p-4 opacity-5"><Wallet size={80} /></div><div className="flex items-center justify-between mb-4 border-b border-white/10 pb-3"><span className="text-[9px] font-black text-indigo-400 uppercase tracking-widest">RÉSUMÉ FINANCIER CHAMBRE</span></div><div className="space-y-3"><div className="flex justify-between items-center text-sm"><span className="font-bold text-slate-400">Total Chambre</span><span className="font-black text-lg">{formatPrice(formData!.totalPrice)}</span></div><div className="flex justify-between items-center text-sm"><span className="font-bold text-slate-400">Total Encaissé</span><span className="font-bold text-emerald-400">{formatPrice(currentResPaid)}</span></div><div className="h-px bg-white/10 my-2"></div><div className="flex justify-between items-center"><span className="font-black text-[10px] text-indigo-300 uppercase">Solde Chambre</span><span className={`text-3xl font-[1000] tabular-nums tracking-tighter ${currentResBalance > 0.05 ? 'text-red-500' : 'text-emerald-500'}`}>{formatPrice(Math.max(0, currentResBalance))}</span></div></div></div>
                                        </div>
                                    </div>
                                </div>
                            )}
            ...
            {isClientModalOpen && <ClientModal client={clientInfo} isOpen={isClientModalOpen} onClose={() => setIsClientModalOpen(false)} onSave={async (updatedClient) => {
                await api.updateClient(updatedClient);
                setClientInfo(updatedClient);

                // Update the reservation's cached client name
                if (formData) {
                    const newName = updatedClient.company || `${updatedClient.firstName} ${updatedClient.lastName}`;
                    const updated = { ...formData, clientName: newName };
                    setFormData(updated);
                    // Save to DB to persist the name change in the reservation record
                    await api.updateReservation(updated);
                        ...
                        <div className="p-8">
                            <div className="text-center mb-8">
                                ...
                                <div className={`text-4xl font-[1000] tabular-nums tracking-tighter ${(checkOutType === 'group' ? groupFinancials.balance : currentResBalance) > 0.05 ? 'text-red-500' : 'text-emerald-500'}`}>
                                    {formatPrice(checkOutType === 'group' ? groupFinancials.balance : currentResBalance)}
                                </div>
                            </div>
