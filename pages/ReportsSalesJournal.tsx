            ...
            <main className="flex-1 max-w-7xl mx-auto w-full p-8 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 no-print">
                    <div className="bg-slate-900 text-white p-6 rounded-2xl shadow-lg relative overflow-hidden">
                        ...
                        <h3 className="text-3xl font-bold">{formatPrice(totals.ht)}</h3>
                    ...
                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                        ...
                        <h3 className="text-3xl font-bold text-slate-800">{formatPrice(totals.tva)}</h3>
                        <div className="mt-4 flex flex-wrap gap-2">
                            {/* Fix: Explicitly type reduce to ensure results are treated as numbers for toFixed calls */}
                            <span className="bg-blue-50 text-blue-600 text-[10px] font-bold px-1.5 py-0.5 rounded border border-blue-100">10%: {formatPrice(filteredSales.filter((l: SalesLine) => l.tvaRate === 0.1).reduce<number>((a, c) => a + c.tvaAmount, 0))}</span>
                            <span className="bg-orange-50 text-orange-600 text-[10px] font-bold px-1.5 py-0.5 rounded border border-orange-100">20%: {formatPrice(filteredSales.filter((l: SalesLine) => l.tvaRate === 0.2).reduce<number>((a, c) => a + c.tvaAmount, 0))}</span>
                        </div>
                    </div>
                    ...
                    <div className="bg-indigo-50 border border-indigo-200 p-6 rounded-2xl shadow-sm">
                        ...
                        <h3 className="text-3xl font-bold text-indigo-700">{formatPrice(totals.ttc)}</h3>
                        ...
                        <div className="mt-2 space-y-2">
                            {Object.entries(breakdown).map(([cat, val]) => (
                                <div key={cat} className="flex items-center justify-between text-sm mb-1">
                                    ...
                                    <span className="font-bold text-slate-800">{formatPrice(val as number)}</span>
                                </div>
                            ))}
                        </div>
                    </div>
                        ...
                        <table className="w-full text-left text-sm border-collapse">
                            ...
                            <tfoot className="bg-white text-slate-900 font-bold border-t border-slate-100 print:bg-white print:text-slate-900 print:border-t-slate-900">
                                <tr className="divide-x divide-slate-100">
                                    ...
                                    <td className="px-6 py-6 text-right text-indigo-600">{formatPrice(totals.ht as number)}</td>
                                    <td className="px-6 py-6"></td>
                                    <td className="px-6 py-6 text-right text-slate-400">{formatPrice(totals.tva as number)}</td>
                                    <td className="px-6 py-6 text-right text-indigo-700 text-xl font-black bg-indigo-50/30">{formatPrice(totals.ttc as number)}</td>
                                </tr>
                            </tfoot>
                        </table>
                    </div>
                            ...
                            <table className="w-full text-xs">
                                <tbody>
                                    <tr className="border-b">
                                        ...
                                        <td className="py-2 text-right font-bold">HT: {formatPrice(salesJournal.filter((l: SalesLine) => l.tvaRate === 0.1).reduce<number>((acc, curr) => acc + curr.ht, 0))}</td>
                                        <td className="py-2 text-right font-bold">TVA: {formatPrice(salesJournal.filter((l: SalesLine) => l.tvaRate === 0.1).reduce<number>((acc, curr) => acc + curr.tvaAmount, 0))}</td>
                                    </tr>
                                    <tr className="border-b">
                                        <td className="py-2 text-slate-500">Ventes de biens / Divers (20%)</td>
                                        {/* Fix: Explicitly specifying generic return type of reduce to avoid 'unknown' type errors for toFixed */}
                                        <td className="py-2 text-right font-bold">HT: {formatPrice(salesJournal.filter((l: SalesLine) => l.tvaRate === 0.2).reduce<number>((acc, curr) => acc + curr.ht, 0))}</td>
                                        <td className="py-2 text-right font-bold">TVA: {formatPrice(salesJournal.filter((l: SalesLine) => l.tvaRate === 0.2).reduce<number>((acc, curr) => acc + curr.tvaAmount, 0))}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
