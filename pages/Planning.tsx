        ...
        <div className="flex items-center justify-between gap-1.5">
          <div className="flex items-center gap-1.5 overflow-hidden">
            ...
            <div className={`${isSmall ? 'text-[9px]' : 'text-[11px]'} font-black truncate leading-none tracking-tight uppercase`}>
              {displayName} <span className={`ml-1 ${settings.barStyle === 'solid' ? 'opacity-70' : 'opacity-60'} font-black tracking-normal`}>[{formatPrice(reservation.totalPrice)}]</span>
            </div>
          </div>
          {!isSmall && (
        ...
        <div className="fixed inset-0 z-[600] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-md overflow-hidden border border-slate-100 animate-in zoom-in-95">
            <div className="bg-red-600 text-white p-6 flex items-center gap-4">
              ...
              <div>
                <h3 className="font-black uppercase tracking-tight text-lg">Réinitialisation</h3>
                <p className="text-[10px] font-bold text-red-100 uppercase tracking-widest">Action Irréversible</p>
              </div>
            </div>
            <div className="p-8 text-center">
              <div className="mb-6 flex flex-col items-center">
                <div className="w-16 h-16 rounded-3xl bg-red-50 border-2 border-red-100 flex items-center justify-center mb-4 text-red-600">
                  <RotateCcw size={32} />
                </div>
                <p className="text-slate-600 text-sm leading-relaxed font-medium">
                  Êtes-vous sûr de vouloir réinitialiser toutes les données du planning ?<br />
                  <span className="font-black text-red-600 mt-2 block uppercase text-[10px] tracking-wider">
                    Les réservations, paiements et factures seront définitivement effacés.
                  </span>
                  <span className="text-slate-400 text-[10px] mt-1 block">
                    (Les fiches clients et les chambres seront conservées)
                  </span>
                </p>
              </div>
            </div>
