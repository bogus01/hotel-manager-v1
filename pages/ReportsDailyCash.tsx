        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-4 space-y-6">
                ...
                <div className="bg-slate-900 text-white p-6 rounded-2xl shadow-xl relative overflow-hidden">
                    ...
                    <h3 className="text-4xl font-bold">{financialData.totalSales.toFixed(2)} €</h3>
                    <div className="mt-4 pt-4 border-t border-slate-800 flex justify-between items-center text-sm">
                        <div className="flex items-center gap-2 text-slate-400">
                            <BedDouble size={16}/> Héb: {financialData.roomSales.toFixed(0)}€
                        </div>
                        <div className="flex items-center gap-2 text-slate-400">
                            <Utensils size={16}/> Extras: {financialData.extraSales.toFixed(0)}€
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="mt-6 pt-4 bg-slate-50 -mx-6 px-6 border-t border-slate-100 flex justify-between items-baseline">
                            ...
                            <span className="text-xl font-black text-emerald-600">{financialData.totalCollected.toFixed(2)} €</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-amber-50 border border-amber-200 p-6 rounded-2xl">
                    ...
                    <div className="flex justify-between items-center mb-4">
                        ...
                        <span className="text-lg font-bold text-amber-900">{(financialData.paymentsByMethod['Espèces'] as number || 0).toFixed(2)} €</span>
                    </div>
                                ...
                                {financialData.transactions.map((t, idx) => (
                                    <tr key={idx} className="hover:bg-slate-50 transition-colors">
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            {t.type === 'sale' ? (
                                                <span className="text-blue-600 font-bold">+{t.amount.toFixed(2)} €</span>
                                            ) : '-'}
                                        </td>
                                        <td className="px-6 py-4 text-right">
                                            {t.type === 'payment' ? (
                                                <span className="text-emerald-600 font-bold">+{t.amount.toFixed(2)} €</span>
                                            ) : '-'}
                                        </td>
                                    </tr>
                                ))}
                    ...
                    <div className="p-5 flex justify-between items-center font-bold text-slate-800">
                        ...
                        <div className="flex gap-8">
                            <div className="text-right">
                                ...
                                <p className="text-blue-700">{financialData.totalSales.toFixed(2)} €</p>
                            </div>
                            <div className="text-right">
                                <p className="text-[10px] text-slate-400 uppercase">Encaissements</p>
                                <p className="text-emerald-700">{financialData.totalCollected.toFixed(2)} €</p>
                            </div>
                        </div>
                    </div>
                </div>
