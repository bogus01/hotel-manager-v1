...
const Billing: React.FC = () => {
    ...
    const stats = useMemo(() => {
        ...
        reservations.forEach(res => {
            ...
            const paymentsArray = Array.isArray(res.payments) ? res.payments : [];

            // Collecté sur la période
            paymentsArray.forEach(pay => {
                if (isWithinInterval(new Date(pay.date), { start, end })) {
            ...
            if (isWithinInterval(res.checkOut, { start, end })) {
                totalInvoiced += Number(res.totalPrice) || 0;
                const paid = paymentsArray.reduce((acc, p) => acc + (Number(p.amount) || 0), 0) + (Number(res.depositAmount) || 0);
                const due = (Number(res.totalPrice) || 0) - paid;
            ...
            if (isAfter(res.checkIn, today) && isWithinInterval(res.checkIn, { start, end })) {
                totalForecast += Number(res.totalPrice) || 0;
            }
        });
    ...
    const allPayments = useMemo(() => {
        ...
        reservations.forEach(res => {
            ...
            paymentsArray.forEach(p => {
                list.push({
                    payment: { ...p, amount: Number(p.amount) || 0 },
                    reservation: res
                });
            });
            const depositAmount = Number(res.depositAmount) || 0;
            if (depositAmount > 0) {
                list.push({
                    payment: { id: `dep-${res.id}`, amount: depositAmount, date: res.checkIn, method: 'Acompte' },
                    reservation: res
                });
            }
        });
    ...
    const filteredInvoices = useMemo(() => {
        ...
        return reservations.filter(res => {
            ...
            if (showDebtorsOnly) {
                const paymentsArray = Array.isArray(res.payments) ? res.payments : [];
                const paid = paymentsArray.reduce((acc, p) => acc + (Number(p.amount) || 0), 0) + (Number(res.depositAmount) || 0);
                matchesDebt = ((Number(res.totalPrice) || 0) - paid) > 0.05;
            }
            ...
            <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-8 max-w-7xl mx-auto w-full">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm group hover:border-indigo-300 transition-all">
                        ...
                        <h3 className="text-2xl font-black text-slate-900 tabular-nums">{formatPrice(stats.totalCollected)}</h3>
                    ...
                    <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm group hover:border-indigo-300 transition-all">
                        ...
                        <h3 className="text-2xl font-black text-slate-900 tabular-nums">{formatPrice(stats.totalInvoiced)}</h3>
                    ...
                    <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm group hover:border-indigo-300 transition-all">
                        ...
                        <h3 className="text-2xl font-black text-red-600 tabular-nums">{formatPrice(stats.totalOutstanding)}</h3>
                    ...
                    <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm group hover:border-indigo-300 transition-all">
                        ...
                        <h3 className="text-2xl font-black text-slate-900 tabular-nums">{formatPrice(stats.totalForecast)}</h3>
                                    ...
                                    {filteredInvoices.map(res => {
                                        ...
                                        return (
                                            <tr key={res.id} className="hover:bg-slate-50 group">
                                                ...
                                                <td className="px-6 py-4 text-right font-black text-slate-900 tabular-nums">{formatPrice(res.totalPrice)}</td>
                                                <td className="px-6 py-4 text-right font-bold text-emerald-600 tabular-nums">-{formatPrice(paid)}</td>
                                ...
                                <tbody className="divide-y divide-slate-100">
                                    {allPayments.map((item, idx) => (
                                        <tr key={idx} className="hover:bg-slate-50 transition-colors group">
                                            ...
                                            <td className="px-6 py-4 text-right font-black text-emerald-600 text-lg tabular-nums">+{formatPrice(item.payment.amount)}</td>
                                            <td className="px-6 py-4 text-right no-print">
                                                <button
                                ...
                                <tbody className="divide-y divide-slate-100">
                                    {filteredForecast.map(res => (
                                        <tr key={res.id} className="hover:bg-slate-50 transition-colors">
                                            ...
                                            <td className="px-6 py-4 text-right font-black text-slate-900 text-lg tabular-nums">{formatPrice(res.totalPrice)}</td>
                                            <td className="px-6 py-4 text-right no-print">
