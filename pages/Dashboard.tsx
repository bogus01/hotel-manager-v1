        ...
        <div className="min-h-screen bg-slate-100 flex flex-col font-sans antialiased text-slate-900">
            ...
            <div className="flex-1 p-8 space-y-8 max-w-[1600px] mx-auto w-full">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    <StatCard title="ADR" value={formatPrice(Math.round(stats.arr))} change="+5" subtext="PRIX MOYEN" icon={<Coins />} color="bg-emerald-500" />
                    <StatCard title="RevPAR" value={formatPrice(Math.round(stats.revpar))} change="+12%" subtext="PERFORMANCE" icon={<BarChart3 />} color="bg-purple-600" />
                    <StatCard title="Revenu Hébergement" value={formatPrice(Math.round(stats.rev))} change="+1.2k" subtext="TOTAL HT" icon={<CreditCard />} color="bg-blue-600" />
                </div>
                    ...
                    <div className="lg:col-span-3 bg-white p-6 rounded-[2.5rem] shadow-sm border border-slate-200 overflow-hidden flex flex-col">
                        ...
                        <div className="flex-1 overflow-auto">
                            <table className="w-full text-left">
                                ...
                                <tbody className="divide-y divide-slate-100">{bobData.map((d, i) => (<tr key={i} className="hover:bg-slate-50 transition-colors"><td className="p-3 text-xs font-black text-slate-700">{d.date}</td><td className="p-3 text-right text-xs font-bold text-emerald-600">{formatPrice(d.revenue)}</td><td className="p-3 text-right"><div className="flex items-center justify-end gap-2"><span className="text-xs font-black text-slate-800">{d.occupancy}%</span><div className="w-1.5 h-6 bg-slate-100 rounded-full overflow-hidden"><div className="bg-indigo-600 w-full" style={{ height: `${d.occupancy}%`, marginTop: `${100 - d.occupancy}%` }}></div></div></div></td></tr>))}</tbody>
                            </table>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-200">
                    <div className="flex justify-between items-center mb-8">
                        <div>
                            ...
                            <p className="text-xs text-slate-400 font-bold uppercase tracking-widest mt-1">Nombre de chambres restantes à vendre par jour</p>
                        </div>
