...
const SettingsRooms: React.FC = () => {
    ...
    const [roomBaseRate, setRoomBaseRate] = useState<number>(0);
    ...
    const handleRoomCreate = () => {
        ...
        setRoomCategoryId(defaultCat?.id || '');
        setRoomFixedCapacity(defaultCat?.defaultCapacity || 2);
        setRoomPricingModel(defaultCat?.pricingModel || 'fixed');
        setRoomBaseRate(defaultCat?.defaultBaseRate || 0);
        setRoomOccupancyPrices(defaultCat?.occupancyPrices || { [defaultCat?.defaultCapacity || 1]: defaultCat?.defaultBaseRate || 0 });
        setIsRoomModalOpen(true);
    };
    ...
    const handleRoomSave = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const isDuplicate = rooms.some(r => r.id !== editingRoom?.id && r.number.trim().toLowerCase() === num.trim().toLowerCase());
        if (isDuplicate) { setRoomModalError(`La chambre n°${num} existe déjà dans l'établissement.`); return; }

        const currentBaseRate = roomPricingModel === 'fixed' ? roomBaseRate : (roomOccupancyPrices[roomFixedCapacity] || 0);
        const roomData = {
            number: num,
            type: typ,
            categoryId: roomCategoryId,
            floor: fl,
            capacity: roomFixedCapacity,
            baseRate: currentBaseRate,
            pricingModel: roomPricingModel,
            occupancyPrices: roomPricingModel === 'fixed' ? { [roomFixedCapacity]: currentBaseRate } : roomOccupancyPrices
        };
        ...
        <div className="min-h-screen bg-slate-50 flex flex-col font-sans antialiased">
            ...
            <div className="flex-1 max-w-7xl mx-auto w-full p-8 space-y-8 pb-32">
                <div className="flex gap-1 bg-white p-1 rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                    <button onClick={() => setActiveTab('rooms')} className={`flex-1 py-3 px-6 rounded-xl flex items-center justify-center gap-3 transition-all font-black text-[10px] uppercase tracking-widest ${activeTab === 'rooms' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:bg-slate-50'}`}><LayoutList size={16} /> Registre Chambres</button>
                    ...
                    <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden animate-in fade-in slide-in-from-bottom-2">
                        <div className="p-6 border-b border-slate-100 bg-slate-50/50 flex flex-col md:flex-row justify-between items-center gap-6">
                            <div>
                                ...
                                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-0.5">Configuration physique et prix par chambre</p>
                            </div>
                            <button onClick={handleRoomCreate} className="bg-indigo-600 text-white px-6 py-2.5 rounded-xl font-black uppercase tracking-widest text-[10px] hover:bg-indigo-700 shadow-lg shadow-indigo-100 transition-all flex items-center gap-2"><Plus size={16} /> Ajouter Chambre</button>
                        </div>
                                ...
                                <tbody className="divide-y divide-slate-100">
                                    {filteredAndSortedRooms.map(room => (
                                        <tr key={room.id} className="hover:bg-slate-50/80 transition-colors group">
                                            ...
                                            <td className="px-6 py-4 text-right font-black text-emerald-600 text-[13px] tabular-nums">{room.pricingModel === 'flexible' ? `Dès ${formatPrice(room.occupancyPrices[1] || room.baseRate || 0)}` : formatPrice(room.baseRate || 0)}</td>
                                ...
                                <tbody className="divide-y divide-slate-100">
                                    {categories.map(cat => (
                                        <tr key={cat.id} className="hover:bg-slate-50 transition-colors group">
                                            ...
                                            <td className="px-6 py-4 text-right font-black text-emerald-600 text-[13px]">{formatPrice(cat.defaultBaseRate || 0)}</td>
                    ...
                    <form onSubmit={handleRoomSave} className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in duration-300 border border-white/20 flex flex-col">
                        <div className="bg-slate-900 text-white p-6 flex justify-between items-center">
                            <div className="flex items-center gap-4">
                                ...
                                <div><h3 className="font-black uppercase tracking-tight text-base">{editingRoom ? 'Configuration Chambre' : 'Nouvelle Chambre'}</h3><p className="text-[9px] font-black text-indigo-400 uppercase tracking-widest mt-0.5">Inventaire chambres</p></div>
                            </div>
                        ...
                        <div className="p-10 space-y-6 overflow-y-auto">
                            ...
                            <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100">
                                {roomPricingModel === 'fixed' ? (
                                    <div><label className={labelClass}>Tarif Fixe par nuit ({currencySettings.symbol})</label><input required name="baseRate" type="number" step="0.01" value={roomBaseRate} onChange={e => setRoomBaseRate(parseFloat(e.target.value) || 0)} className={`${inputClass} !bg-white text-lg h-12 text-emerald-600`} /></div>
                                ) : (
                        ...
                        <div className="p-8 bg-slate-50 border-t border-slate-100 flex justify-end gap-4">
                            ...
                            <button type="submit" className="bg-indigo-600 text-white px-8 py-3 rounded-xl font-black uppercase tracking-widest text-[10px] shadow-lg shadow-indigo-100 hover:bg-indigo-700 flex items-center gap-2"><Check size={18} /> Enregistrer la chambre</button>
                        </div>
                    </form>
                </div>
            )}
                        ...
                        <div className="p-10 space-y-6 overflow-y-auto">
                            ...
                            <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100">
                                {catPricingModel === 'fixed' ? (
                                    <div><label className={labelClass}>Prix de base par nuit ({currencySettings.symbol})</label><input required name="defaultBaseRate" type="number" step="0.01" defaultValue={editingCat?.defaultBaseRate} className={`${inputClass} !bg-white text-lg h-12 text-emerald-600`} /></div>
                                ) : (
                                    <div className="space-y-4">
            ...
            {deletingRoomId && (
                <div className="fixed inset-0 z-[600] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
                    <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-sm overflow-hidden border border-slate-100 p-8 text-center animate-in zoom-in duration-300">
                        ...
                        <h3 className="font-black text-slate-800 uppercase tracking-tight text-lg mb-2">Supprimer la chambre ?</h3>
